...
func GetAllUsers() ([]*defs.User, error) {
	stmtOut, err := dbConn.Prepare(`SELECT users.id,users.login_name,users.pwd FROM users`)
	if err != nil {
		log.Printf("%v", err)
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &loginName, &pwd); err != nil {
			return nil, err
		}
		user := &defs.User{
			Id:        id,
...
func ListVideoInfo( /*uname string,*/ from, to int) ([]*defs.VideoInfo, error) {
	stmtOut, err := dbConn.Prepare(`SELECT video_info.id, video_info.author_id, video_info.name, video_info.display_ctime FROM video_info /*INNER JOIN users ON video_info.author_id = users.id*/ WHERE /*users.login_name=? AND */video_info.create_time > FROM_UNIXTIME(?) AND video_info.create_time<=FROM_UNIXTIME(?) ORDER BY video_info.create_time DESC`)
